...
日志区会实时显示进度。两个勾选项（**默认值在 `epub2pdf_gui.py` 第 138 / 140 行的 `value=` 处改**）：

| 勾选项 | 默认 | 作用 |
| --- | --- | --- |
| 转完自动打开 PDF | ✗ 不勾 | 转完用系统默认阅读器打开成品（批量转的时候不勾更省事） |
| 转完删除中间文件（含 `_work` 整目录） | ✔ 勾上 | 一轮转换全部结束后，把整个 `_work` 目录删掉（含各书中间文件、`_gui_run.log`）；下次转换会自动重建。**要留现场排错就先取消勾选** |
